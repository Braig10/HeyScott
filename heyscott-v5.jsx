...
const C = {
  ...
  lav: "#C4BCEE", lavSoft: "#B8B0E8", lavPale: "#EDE9FE",
  bg: "#F2EDE8", bgDeep: "#EDEAE6", white: "#FFFFFF",
  border: "#E4DDD6", muted: "#6B6580", text: "#140F2D",
...
function Ring({score,sz=80}){
  ...
  const col=score>=80?"#22C55E":score>=70?"#F59E0B":C.purple;
  return(
    <svg width={sz} height={sz} style={{transform:"rotate(-90deg)"}}>
      ...
      <text x={sz/2} y={sz/2} textAnchor="middle" dominantBaseline="middle" style={{transform:`rotate(90deg)`,transformOrigin:`${sz/2}px ${sz/2}px`,fontFamily:"DM Sans",fontSize:sz*0.25,fontWeight:800,fill:col}}>{score}</text>
    </svg>
  );
}
...
function LeftPanel({go}){
  return(
    <aside style={{width:220,flexShrink:0,display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {[{ic:"⭐",label:"Avg. Score",val:"75"},{ic:"✅",label:"Lessons Done",val:"2"},{ic:"⚡",label:"Day Streak",val:"5"}].map((s,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:32,height:32,borderRadius:10,background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{s.ic}</div>
      ...
      <button onClick={()=>go("analysis")} style={{background:C.purple,color:"#fff",border:"none",borderRadius:12,padding:"10px 14px",fontSize:13,fontWeight:700,cursor:"pointer",textAlign:"left"}}>
        🎯 Ask Scott
      </button>
      <div style={{borderTop:`1px solid ${C.border}`}}/>
...
function Landing({go}){
  ...
  const slides=[
    {
      bg:C.navy, tag:"AI Sales Coaching",
      headline:"Turn Every Call\nInto Your Next Win",
      sub:"AI coaching built for recruiters — feedback, milestones, and mindset in one place.",
      cta:"Start free →", cta2:"See how it works",
      icon:"📞", shape:"hero",
    },
    {
      bg:"#5C5490", tag:"Upload & Analyse",
      headline:"Real Feedback\nOn Real Calls",
      sub:"Upload a recording or paste a transcript. Get scored feedback on consultative selling, listening, objections and more.",
      cta:"Ask Scott →", cta2:null,
      icon:"📊", shape:"oval",
    },
    {
      bg:C.navy, tag:"AI Roleplay",
      headline:"Practice With AI\nBefore the Real Thing",
      sub:"Roleplay cold calls with AI candidates — The Gatekeeper, The Fence-Sitter, and more. Instant coaching after every session.",
      cta:"Start Learning →", cta2:null,
      icon:"🎯", shape:"oval",
    },
    {
      bg:C.purple, tag:"Track Progress",
      headline:"Build Capability\nWeek by Week",
      sub:"Build the mental and emotional capability to make more calls, stay consistent, and show up as your best self every day.",
      cta:"Open Journal →", cta2:null,
      icon:"🚀", shape:"oval",
    },
    {
      bg:C.lav, tag:"For Managers", tagDark:true,
      headline:"Full Team\nVisibility",
      sub:"See who's improving, who's at risk, and where to coach next. Manager dashboard built for a 5-minute weekly check-in.",
      cta:"Explore →", cta2:null,
      icon:"👥", shape:"oval", dark:true,
    },
  ];
  ...
  const features=[
    ...
    {icon:"🎭",title:"AI Roleplay",desc:"Practice with named personas: The Gatekeeper, The Fence-Sitter."},
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'DM Sans',sans-serif"}}>
      {/* Header */}
      <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"18px 32px",position:"sticky",top:0,background:C.bg,zIndex:10,borderBottom:`1px solid ${C.border}`}}>
        <div style={{fontWeight:800,fontSize:20,color:C.navy}}>Hey<span style={{color:C.purple}}>Scott</span></div>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          <button onClick={()=>go("onboarding")} style={{background:C.navy,color:"#fff",border:"none",borderRadius:999,padding:"9px 20px",fontSize:13,fontWeight:700,cursor:"pointer"}}>Get started</button>
        </div>
      </header>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"40px 24px 0"}}>
        {/* Slide */}
        <div key={slide} style={{borderRadius:32,background:cur.bg,padding:"48px 40px",marginBottom:20,transition:"all 0.3s",minHeight:380,display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
          <div>
            <span style={{background:cur.dark?"rgba(20,15,45,0.15)":"rgba(255,255,255,0.15)",color:cur.dark?C.navy:"rgba(255,255,255,0.9)",borderRadius:999,padding:"5px 14px",fontSize:12,fontWeight:700,display:"inline-block",marginBottom:24}}>
              {cur.tag}
            </span>
            <div style={{fontSize:"clamp(28px,5vw,44px)",fontWeight:900,color:cur.dark?C.navy:"#fff",lineHeight:1.1,whiteSpace:"pre-line",marginBottom:20,letterSpacing:-1}}>
              {cur.headline}
            </div>
            <p style={{fontSize:15,color:cur.dark?"rgba(20,15,45,0.65)":"rgba(255,255,255,0.72)",lineHeight:1.65,maxWidth:480}}>
              {cur.sub}
            </p>
          </div>
          <div style={{display:"flex",gap:12,marginTop:32,flexWrap:"wrap",alignItems:"center"}}>
            <button onClick={()=>go("onboarding")} style={{background:cur.dark?C.navy:C.lav,color:cur.dark?"#fff":C.navy,border:"none",borderRadius:999,padding:"12px 24px",fontWeight:800,cursor:"pointer",fontSize:13}}>
              {cur.cta}
            </button>
            {cur.cta2&&<button style={{background:"none",color:cur.dark?C.navy:"rgba(255,255,255,0.8)",border:`1.5px solid ${cur.dark?"rgba(20,15,45,0.3)":"rgba(255,255,255,0.4)"}`,borderRadius:999,padding:"11px 20px",fontWeight:600,cursor:"pointer",fontSize:13}}>
              {cur.cta2}
            </button>}
            <div style={{fontSize:36,marginLeft:"auto",opacity:0.35}}>{cur.icon}</div>
          </div>
        </div>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:32}}>
          {slides.map((_,i)=>(
            <button key={i} onClick={()=>setSlide(i)} style={{width:i===slide?24:8,height:8,borderRadius:999,background:i===slide?C.navy:C.border,border:"none",cursor:"pointer",transition:"all 0.2s",padding:0}}/>
          ))}
        </div>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:48}}>
          <button onClick={()=>setSlide(s=>Math.max(0,s-1))} disabled={slide===0}
            style={{width:40,height:40,borderRadius:"50%",border:`1px solid ${C.border}`,background:C.white,cursor:slide===0?"not-allowed":"pointer",opacity:slide===0?0.4:1,fontSize:16}}>←</button>
          <button onClick={()=>setSlide(s=>Math.min(slides.length-1,s+1))} disabled={slide===slides.length-1}
            style={{width:40,height:40,borderRadius:"50%",border:`1px solid ${C.border}`,background:C.white,cursor:slide===slides.length-1?"not-allowed":"pointer",opacity:slide===slides.length-1?0.4:1,fontSize:16}}>→</button>
        </div>
      </div>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"0 24px 40px"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:C.muted,marginBottom:12}}>Everything you need</div>
          <div style={{fontSize:28,fontWeight:900,color:C.navy,letterSpacing:-0.5}}>Built for the recruitment floor</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14}}>
          {features.map((f,i)=>(
            <div key={i} style={{background:C.white,borderRadius:20,padding:22,border:`1px solid ${C.border}`}}>
      ...
      {/* Stats band */}
      <div style={{background:C.navy,padding:"40px 24px",margin:"0 0 40px"}}>
        <div style={{maxWidth:640,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:24,textAlign:"center"}}>
          {[{n:"+34%",l:"Faster onboarding"},{n:"<8 min",l:"To first roleplay"},{n:"32%",l:"Industry churn — we fix it"}].map((s,i)=>(
            <div key={i}><div style={{fontSize:32,fontWeight:900,color:C.lav}}>{s.n}</div><div style={{fontSize:13,color:"rgba(255,255,255,0.55)",marginTop:4}}>{s.l}</div></div>
          ))}
        </div>
      </div>
      ...
      <div style={{maxWidth:680,margin:"0 auto",padding:"0 24px 80px",textAlign:"center"}}>
        <div style={{background:C.lav,borderRadius:32,padding:"48px 40px"}}>
          <div style={{fontSize:32,fontWeight:900,color:C.navy,letterSpacing:-0.5,marginBottom:12}}>Ready to level up?</div>
          <p style={{fontSize:15,color:"rgba(20,15,45,0.65)",marginBottom:28,lineHeight:1.6}}>Start your free week. No setup. No credit card. Just better calls.</p>
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={()=>go("onboarding")} style={{background:C.navy,color:"#fff",border:"none",borderRadius:999,padding:"14px 32px",fontWeight:800,cursor:"pointer",fontSize:14}}>Sign up free</button>
            <button onClick={()=>go("onboarding")} style={{background:"rgba(20,15,45,0.08)",color:C.navy,border:"none",borderRadius:999,padding:"14px 28px",fontWeight:700,cursor:"pointer",fontSize:14}}>Sign in</button>
          </div>
          <div style={{display:"flex",justifyContent:"center",gap:24,marginTop:20,fontSize:12,color:"rgba(20,15,45,0.5)",flexWrap:"wrap"}}>
...
function Onboarding({go,setProfile,setUserRole}){
  ...
  return <ScottOnboarding existingProfile={null} onComplete={p=>{setProfile(p);saveProfile(p);setUserRole("learner");go("learning");}}/>;
}
...
- Arc: acknowledge energy → surface the specific behaviour → get underneath it → reframe → one practical tool → one tiny commitment for today`;
  ...
  return(
    <div style={{animation:"fadeUp 0.3s ease both"}}>
      {/* Tab bar */}
      <div style={{display:"flex",gap:4,marginBottom:20,background:C.white,borderRadius:12,padding:4,border:`1px solid ${C.border}`,width:"fit-content"}}>
        {TABS.map(t=>(
        ...
        <div>
          ...
          <div style={{display:"flex",gap:20,alignItems:"flex-start",marginBottom:20,flexWrap:"wrap"}}>
            ...
            <div style={{display:"flex",gap:10,flexShrink:0}}>
              {streak>0&&(
                <div style={{background:C.navy,borderRadius:12,padding:"10px 16px",textAlign:"center"}}>
                  <div style={{fontSize:20,fontWeight:900,color:"#fff"}}>{streak}</div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.5)"}}>day streak</div>
                </div>
              )}
              {avgEnergy&&(
                <div style={{background:C.white,borderRadius:12,padding:"10px 16px",textAlign:"center",border:`1px solid ${C.border}`}}>
          ...
          {todayEntry && mode==="idle" && (
            <div style={{background:"#F0FDF4",borderRadius:16,border:"1px solid #BBF7D0",padding:"18px 22px",marginBottom:16}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8}}>
                {[1,2,3,4,5].map(e=>(
                  <button key={e} onClick={()=>startSession(e)}
                    style={{background:C.bg,border:`2px solid ${C.border}`,borderRadius:14,padding:"14px 8px",cursor:"pointer",textAlign:"center",transition:"all 0.15s",fontFamily:"'DM Sans',sans-serif"}}
          ...
          {mode==="chat" && (
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              {/* Energy badge */}
              {energy&&(
                <div style={{display:"flex",gap:8,alignItems:"center",padding:"8px 14px",background:C.bg,borderRadius:10,width:"fit-content"}}>
              ...
              {/* Chat area */}
              <div ref={chatRef} style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:16,minHeight:360,maxHeight:480,overflowY:"auto",display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)}
                  ...
                  disabled={loading}
                  style={{flex:1,background:C.white,border:`1px solid ${C.border}`,borderRadius:999,padding:"11px 18px",fontSize:13,color:C.text,outline:"none",fontFamily:"'DM Sans',sans-serif",opacity:loading?0.6:1}}/>
          ...
          {/* Done */}
          {mode==="done" && (
            <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"24px 22px",textAlign:"center"}}>
      ...
      {tab==="Progress" && (
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          {entries.length < 3 ? (
            ...
            <>
              {/* Energy trend */}
              <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"18px 22px"}}>
  ...
  return(
    <Shell page="learning" go={go} userRole="learner">
      <div style={{animation:"fadeUp 0.35s ease both"}}>
        ...
        {profile?.challenge&&(
          <div style={{background:C.navy,borderRadius:16,padding:"16px 20px",marginBottom:16,display:"flex",gap:14,alignItems:"flex-start"}}>
    ...
    <Shell page="learning" go={go} userRole="learner">
      <div style={{animation:"fadeUp 0.35s ease both"}}>
        ...
        <div style={{background:C.navy,borderRadius:18,padding:24,marginBottom:18}}>
          ...
          <div style={{background:"rgba(255,255,255,0.08)",borderRadius:12,padding:"11px 16px",display:"flex",alignItems:"flex-start",gap:10}}>
    ...
    return(
      <div style={{background:C.white,borderRadius:20,border:`1px solid ${C.border}`,padding:"28px 24px",maxWidth:520,margin:"0 auto",textAlign:"center",animation:"fadeUp 0.3s ease both"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:20,textAlign:"left"}}>
          {questions.map(q=>(
            <div key={q.id} style={{display:"flex",gap:10,alignItems:"center",background:C.bg,borderRadius:10,padding:"8px 14px"}}>
              <div style={{width:28,height:28,borderRadius:8,background:ratingBg(answers[q.id]),border:`1px solid`,borderColor:answers[q.id]<=2?"#FECACA":answers[q.id]===3?"#FDE68A":"#BBF7D0",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:14,color:ratingColor(answers[q.id]),flexShrink:0}}>
                {answers[q.id]}
              </div>
              ...
              {[1,2,3,4,5].map(r=>{
                ...
                return(
                  <button key={r} onClick={()=>setAnswers(prev=>({...prev,[q.id]:r}))}
                    style={{flex:1,padding:"10px 4px",borderRadius:12,border:`2px solid ${selected?ratingColor(r):C.border}`,background:selected?ratingBg(r):C.bg,cursor:"pointer",transition:"all 0.15s",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
        <div style={{
          ...
          <div style={{
            ...
            display:"flex",alignItems:"center",justifyContent:"column",
  ...
  if(phase==="brief") return(
    <Shell page="learning" go={go} userRole="learner">
      ...
      <div style={{maxWidth:680}}>
        ...
        <div style={{background:C.navy,borderRadius:16,padding:"16px 20px",marginBottom:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:18}}>
          ...
          <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"16px 18px"}}>
            ...
            <div style={{background:C.lavPale,borderRadius:10,padding:"10px 12px"}}>
        ...
        {/* Voice setup note */}
        <div style={{background:"#F0FDF4",borderRadius:12,border:"1px solid #BBF7D0",padding:"12px 16px",marginBottom:20,display:"flex",gap:10,alignItems:"center"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"300px 1fr",minHeight:"calc(100vh - 60px)",fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <div style={{background:C.white,borderRight:`1px solid ${C.border}`,padding:"24px 20px",overflowY:"auto",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            ...
            <div style={{background:"#FFF7ED",borderRadius:10,padding:"8px 12px",border:"1px solid #FED7AA"}}>
          ...
          <div>
            ...
            {brief.briefType==="limited" && brief.unknowns?.length>0 && (
              <div style={{background:"#FEF3C7",borderRadius:8,padding:"8px 10px",marginTop:4,border:"1px solid #FDE68A"}}>
          ...
          <div>
            ...
            <div style={{background:C.navy,borderRadius:10,padding:"10px 14px",marginBottom:8}}>
            ...
            {scenario.difficulty!=="beginner"&&(
              <div style={{background:scenario.difficulty==="advanced"?"#DBEAFE":"#FEF3C7",borderRadius:8,padding:"6px 10px",border:`1px solid ${scenario.difficulty==="advanced"?"#BFDBFE":"#FDE68A"}`,fontSize:10,fontWeight:700,color:scenario.difficulty==="advanced"?"#1E40AF":"#92400E",textAlign:"center"}}>
        ...
        <div style={{
          ...
          {/* Mic error */}
          {micError&&(
            <div style={{background:"rgba(239,68,68,0.15)",borderRadius:10,padding:"8px 14px",border:"1px solid rgba(239,68,68,0.3)",fontSize:11,color:"#FCA5A5",marginBottom:10,maxWidth:380,textAlign:"center"}}>
              {micError}
            </div>
          )}

          {/* Live transcript */}
          <div style={{width:"100%",maxWidth:420,minHeight:40,background:"rgba(255,255,255,0.06)",borderRadius:10,border:`1px solid ${isRecording?"rgba(96,165,250,0.5)":"rgba(255,255,255,0.08)"}`,padding:"8px 14px",marginBottom:12,transition:"border-color 0.3s"}}>
            {isRecording?(
              <>
          ...
          <div ref={messagesRef} style={{width:"100%",maxWidth:420,flex:"0 1 220px",overflowY:"auto",display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
            {msgs.map((m,i)=>(
              <div key={i}>
                {m.role==="coach"?(
                  <div style={{background:"rgba(196,188,238,0.12)",borderRadius:8,padding:"5px 10px",fontSize:10,color:C.lav,textAlign:"center"}}>💬 {m.content}</div>
                ):(
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,width:"100%",maxWidth:420}}>
            ...
            <div style={{display:"flex",gap:7,width:"100%"}}>
              <input value={textInput} onChange={e=>setTextInput(e.target.value)}
                ...
                style={{flex:1,background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:999,padding:"9px 14px",fontSize:12,color:"rgba(255,255,255,0.9)",outline:"none",fontFamily:"'DM Sans',sans-serif",opacity:callEnded||isRecording?0.3:1}}
              />
        ...
        {!showPostCheck && (
          ...
          {activeDebriefTab==="overview" && (
            <div style={{animation:"fadeUp 0.3s ease both"}}>
                      ...
                      {qdepth && (
                        <div style={{background:C.white,borderRadius:14,padding:"14px 16px",border:`1px solid ${C.border}`}}>
                          ...
                          <div style={{display:"flex",gap:10,marginBottom:6}}>
                            <div style={{background:"#FEE2E2",borderRadius:8,padding:"4px 10px",fontSize:11,fontWeight:700,color:C.red}}>Surface: {qdepth.surfaceCount}</div>
                            <div style={{background:C.greenBg,borderRadius:8,padding:"4px 10px",fontSize:11,fontWeight:700,color:C.green}}>Deep: {qdepth.deepCount}</div>
                          </div>
              ...
              {/* ── STRONGEST MOMENT ── */}
              {result.strongMoment && (
                <div style={{background:"#F0FDF4",borderRadius:16,padding:"18px 22px",border:"1px solid #BBF7D0",marginBottom:12}}>
                  ...
                  <div style={{background:"rgba(255,255,255,0.7)",borderRadius:10,padding:"10px 16px",border:"1px solid #BBF7D0",marginBottom:12}}>
                  ...
                  {result.strongMoment.howToRepeat && (
                    <div style={{background:"rgba(255,255,255,0.6)",borderRadius:8,padding:"8px 12px",border:"1px solid #BBF7D0"}}>
                    ...
                    {(result.missedMoment.recruiterLine||result.missedMoment.candidateLine) ? (
                      <>
                        {result.missedMoment.candidateLine && (
                          <div style={{background:"rgba(255,255,255,0.7)",borderRadius:10,padding:"10px 14px",border:"1px solid #FED7AA"}}>
                      ...
                      <div style={{gridColumn:"1/-1",background:"rgba(255,255,255,0.7)",borderRadius:10,padding:"10px 14px",border:"1px solid #FED7AA"}}>
              ...
              {result.questioningCoach && (
                <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"18px 22px",marginBottom:12}}>
                  ...
                  {result.questioningCoach.questions?.length > 0 && (
                    <div style={{marginBottom:16}}>
                      ...
                      {result.questioningCoach.questions.map((q,i)=>{
                        ...
                        return(
                          <div key={i} style={{background:C.bg,borderRadius:12,padding:"14px 16px",marginBottom:8,border:`1px solid ${C.border}`}}>
                            {/* Asked */}
                            ...
                            {/* Better version */}
                            <div style={{background:C.lavPale,borderRadius:8,padding:"10px 14px",border:`1px solid ${C.lavSoft}`}}>
                  ...
                  {result.questioningCoach.keyPattern && (
                    <div style={{background:"#FEF2F2",borderRadius:10,padding:"10px 14px",border:"1px solid #FECACA",marginBottom:14}}>
                  ...
                  {result.questioningCoach.technique && (
                    <div style={{background:C.navy,borderRadius:12,padding:"16px 18px"}}>
                      ...
                      <div style={{background:"rgba(255,255,255,0.07)",borderRadius:10,padding:"10px 14px",marginBottom:10}}>
                      ...
                      <div style={{background:"rgba(196,188,238,0.2)",borderRadius:10,padding:"10px 14px"}}>
                ...
                <div style={{background:C.white,borderRadius:14,border:"1px solid #FECACA",padding:18,marginBottom:14}}>
                  ...
                  {result.missedOpportunities.map((mo,i)=>(
                    <div key={i} style={{marginBottom:i<result.missedOpportunities.length-1?18:0,paddingBottom:i<result.missedOpportunities.length-1?18:0,borderBottom:i<result.missedOpportunities.length-1?`1px solid ${C.bg}`:"none"}}>
                      ...
                      <div style={{background:"#F0FDF4",borderRadius:10,padding:"10px 13px",border:"1px solid #BBF7D0"}}>
                ...
                <div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:18,marginBottom:14}}>
                  ...
                  {result.capturedOpportunities.map((co,i)=>(
                    <div key={i} style={{marginBottom:i<result.capturedOpportunities.length-1?18:0,paddingBottom:i<result.capturedOpportunities.length-1?18:0,borderBottom:i<result.capturedOpportunities.length-1?`1px solid ${C.bg}`:"none"}}>
                      ...
                      <div style={{background:C.lavPale,borderRadius:10,padding:"10px 13px",border:`1px solid ${C.lavSoft}`}}>
                ...
                <div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:18,marginBottom:14}}>
                  ...
                  {result.goodResponses.map((gr,i)=>(
                    <div key={i} style={{marginBottom:i<result.goodResponses.length-1?18:0,paddingBottom:i<result.goodResponses.length-1?18:0,borderBottom:i<result.goodResponses.length-1?`1px solid ${C.bg}`:"none"}}>
                      ...
                      <div style={{background:C.lavPale,borderRadius:8,padding:"8px 12px",border:`1px solid ${C.lavSoft}`}}>
                      ...
                      <div key={i} style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:"14px 18px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
                          ...
                          <div style={{textAlign:"center",flexShrink:0}}>
                            <div style={{background:bg,border:`1px solid ${border}`,borderRadius:10,padding:"6px 14px"}}>
          ...
          {activeDebriefTab==="frameworks" && (
            <div style={{animation:"fadeUp 0.3s ease both"}}>
              <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"18px 22px",marginBottom:14}}>
                  ...
                  {msgs.filter(m=>m.role!=="coach").map((m,i)=>(
                    <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                      ...
                      <div style={{
                        ...
                        borderRadius:10,padding:"8px 12px",fontSize:13,color:C.text,lineHeight:1.6
                      }}>{m.content}</div>
                    </div>
                  ))}
                </div>
          ...
          {/* ── PASS/FAIL + ACTIONS ── */}
          {!pass && (
            <div style={{background:"#FEE2E2",borderRadius:12,padding:14,marginBottom:16,fontSize:13,color:"#991B1B",border:"1px solid #FECACA",textAlign:"center"}}>
          ...
          {(() => {
            ...
            if (showGoals) return (
              <div>
                <div style={{background:C.amberBg,borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",gap:10,alignItems:"center",border:"1px solid #FDE68A"}}>
    ...
    <div>
      ...
      {goals && !generating && (<>
          ...
          return (<div key={i} style={{background:C.white,borderRadius:14,border:`1px solid ${ps.border}`,marginBottom:10,overflow:"hidden"}}>
            ...
            {isOpen && (<div style={{padding:"0 18px 18px",borderTop:`1px solid ${ps.border}`}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:14,marginBottom:12}}>
                {[{label:"Specific",val:g.specific,icon:"🎯"},{label:"Measurable",val:g.measurable,icon:"📊"},{label:"Achievable",val:g.achievable,icon:"✓"},{label:"Relevant",val:g.relevant,icon:"⚡"}].map(({label,val,icon})=>(<div key={label} style={{background:C.bg,borderRadius:10,padding:"10px 13px"}}><div style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>{icon} {label}</div><div style={{fontSize:12,color:C.text,lineHeight:1.55}}>{val}</div></div>))}
              </div>
              <div style={{background:C.bg,borderRadius:10,padding:"8px 12px",marginBottom:12}}><span style={{fontSize:10,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1}}>⏱ Time-bound: </span><span style={{fontSize:12,color:C.text}}>{g.timebound}</span></div>
              <div style={{background:C.lavPale,borderRadius:10,padding:"10px 14px",display:"flex",gap:10,alignItems:"flex-start"}}><Av ini="SC" col={C.lavSoft} sz={26}/><p style={{fontSize:12,color:C.navy,lineHeight:1.65,margin:0,fontStyle:"italic"}}>"{g.coachNote}"</p></div>
            </div>)}
          </div>);
        })}
        {goals.longTermArc && (<div style={{background:C.navy,borderRadius:14,padding:"16px 20px",marginTop:14}}>
          ...
          <div style={{background:"rgba(255,255,255,0.07)",borderRadius:10,padding:"10px 14px",marginBottom:8}}><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>3-month goal</div><p style={{fontSize:12,color:"rgba(255,255,255,0.85)",lineHeight:1.55,margin:0}}>{goals.longTermArc.arcGoal}</p></div>
          <div style={{background:"rgba(196,188,238,0.15)",borderRadius:10,padding:"10px 14px"}}><div style={{fontSize:10,color:C.lav,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Signal to watch for</div><p style={{fontSize:12,color:"rgba(255,255,255,0.7)",lineHeight:1.55,margin:0}}>{goals.longTermArc.signalToWatch}</p></div>
        </div>)}
      </>)}
    </div>
  );
            ...
            {q.options.map(opt=>{
              ...
              return(
                <button key={opt.id} onClick={()=>select(opt.id)}
                  style={{background:active?C.lavPale:C.bg,border:`2px solid ${active?C.purple:C.border}`,borderRadius:12,padding:"12px 16px",textAlign:"left",cursor:"pointer",fontSize:13,color:active?C.navy:C.text,lineHeight:1.5,transition:"all 0.15s",fontFamily:"'DM Sans',sans-serif",fontWeight:active?600:400}}>
                  {opt.text}
                </button>
              );
            })}
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{maxWidth:480,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{width:52,height:52,borderRadius:16,background:C.purple,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 12px",fontSize:22}}>🎯</div>
        ...
        {currentStep && (
          <div style={{background:C.white,borderRadius:20,padding:"28px 24px 24px",border:`1px solid ${C.border}`,marginBottom:16}}>
            ...
            <input
              ...
              autoFocus
              style={{width:"100%",background:C.bg,border:`2px solid ${C.border}`,borderRadius:12,padding:"12px 16px",fontSize:14,color:C.text,outline:"none",fontFamily:"'DM Sans',sans-serif",boxSizing:"border-box",transition:"border-color 0.2s"}}
            />
          </div>
        )}

        {isLast && assessmentDone && (
          <div style={{background:"#F0FDF4",borderRadius:12,padding:"10px 14px",border:"1px solid #BBF7D0",marginBottom:14,fontSize:12,color:C.green,fontWeight:600}}>
  ...
  return(
    <Shell page="analysis" go={go} userRole="learner">
      <div style={{maxWidth:820}}>
        ...
        {/* Section tabs */}
        <div style={{display:"flex",gap:4,marginBottom:24,background:C.white,borderRadius:12,padding:4,border:`1px solid ${C.border}`,width:"fit-content"}}>
        ...
        {activeSection==="review" && (
          <div>
            ...
            {analysisError && (
              <div style={{background:"#FEF2F2",borderRadius:12,border:"1px solid #FECACA",padding:"12px 16px",marginBottom:14,display:"flex",gap:10,alignItems:"flex-start"}}>
                <div style={{flex:1}}>
              ...
              {showTranscript&&(
                <>
                  <textarea value={transcript} onChange={e=>setTranscript(e.target.value)} rows={14}
                    ...
                    style={{width:"100%",fontSize:12,color:C.text,background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",outline:"none",resize:"vertical",fontFamily:"monospace",lineHeight:1.7,boxSizing:"border-box"}}/>
            ...
            {loading && (
              <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:48,textAlign:"center",marginBottom:16}}>
            ...
            {result && (
              <div style={{animation:"fadeUp 0.3s ease both",marginTop:4}}>
                ...
                {/* ── TABS ── */}
                <div style={{display:"flex",gap:3,marginBottom:16,background:C.white,borderRadius:12,padding:4,border:`1px solid ${C.border}`,width:"fit-content",overflowX:"auto"}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginTop:4}}>
                        {result.topWin&&(
                          <div style={{background:"#F0FDF4",borderRadius:14,border:"1px solid #BBF7D0",padding:"14px 16px"}}>
                            ...
                            <div style={{background:"rgba(255,255,255,0.7)",borderRadius:8,padding:"7px 11px",border:"1px solid #BBF7D0",marginBottom:8,fontSize:12,color:"#14532D",fontStyle:"italic",fontWeight:600}}>"{result.topWin.quote}"</div>
                        ...
                        {result.topMiss&&(
                          <div style={{background:"#FFF7ED",borderRadius:14,border:"1px solid #FED7AA",padding:"14px 16px"}}>
                            ...
                            {result.topMiss.candidateLine&&<div style={{background:"rgba(255,255,255,0.6)",borderRadius:7,padding:"6px 10px",border:"1px solid #FED7AA",marginBottom:6,fontSize:11,color:"#7C2D12",fontStyle:"italic"}}>They said: "{result.topMiss.candidateLine}"</div>}
                            <div style={{background:"#F0FDF4",borderRadius:7,padding:"6px 10px",border:"1px solid #BBF7D0",fontSize:12,color:"#14532D",fontStyle:"italic",fontWeight:600}}>Try: "{result.topMiss.betterResponse}"</div>
                          </div>
                        )}
                      </div>
                        ...
                        {levels.map((level,i)=>{
                          ...
                          return(
                            <div key={level.key} style={{margin:"0 auto",width,marginBottom:8,transition:"width 0.4s"}}>
                              <div style={{background:bg,border:`2px solid ${bdr}`,borderRadius:12,padding:"12px 16px",position:"relative"}}>
                                ...
                                {data?.example&&<div style={{background:"rgba(255,255,255,0.7)",borderRadius:8,padding:"7px 11px",fontSize:11,color:"#14532D",fontStyle:"italic",marginBottom:data.nextLevel?8:0}}>"{data.example}"</div>}
                                ...
                                {reached&&data?.nextLevel&&(
                                  <div style={{background:"rgba(255,255,255,0.5)",borderRadius:8,padding:"6px 10px",border:"1px solid #BBF7D0"}}>
                ...
                {resultTab==="scripts" && (
                  <div style={{animation:"fadeUp 0.3s ease both",display:"flex",flexDirection:"column",gap:12}}>
                    <div style={{background:C.lavPale,borderRadius:12,border:`1px solid ${C.lavSoft}`,padding:"10px 14px",fontSize:12,color:C.purple,lineHeight:1.6}}>
                      ...
                      <div key={i} style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:"16px 20px"}}>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",gap:12,alignItems:"start",marginBottom:g.why?12:0}}>
                          <div>
                            ...
                            <div style={{background:C.bg,borderRadius:8,padding:"10px 13px",fontSize:12,color:C.text,fontStyle:"italic",lineHeight:1.55}}>"{g.current}"</div>
                          </div>
                          ...
                          <div>
                            ...
                            <div style={{background:C.lavPale,borderRadius:8,padding:"10px 13px",border:`1px solid ${C.lavSoft}`,fontSize:12,color:C.navy,fontStyle:"italic",fontWeight:600,lineHeight:1.55}}>"{g.improved}"</div>
                          </div>
                        </div>
        ...
        {activeSection==="ask" && (
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            {/* Chat area */}
            <div ref={chatRef} style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"16px",minHeight:320,maxHeight:500,overflowY:"auto",display:"flex",flexDirection:"column",gap:10}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8,paddingTop:8}}>
                  ...
                  {STARTER_PROMPTS.map((p,i)=>(
                    <button key={i} onClick={()=>sendCoachMessage(p)}
                      style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 14px",textAlign:"left",fontSize:12,color:C.text,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",transition:"background 0.15s"}}>
                      {p}
                    </button>
                  ))}
                </div>
            ...
            <div style={{display:"flex",gap:8}}>
              <input
                ...
                style={{flex:1,background:C.white,border:`1px solid ${C.border}`,borderRadius:999,padding:"11px 18px",fontSize:13,color:C.text,outline:"none",fontFamily:"'DM Sans',sans-serif",opacity:chatLoading?0.6:1}}
              />
  ...
  return(
    <Shell page="progress" go={go} userRole="learner">
      <div style={{maxWidth:760}}>
        ...
        {/* Module progress */}
        <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:22,marginBottom:20}}>
    ...
    <Shell page="analytics" go={go} userRole="learner">
      <div style={{maxWidth:760}}>
        ...
        {roleplays.length === 0 ? (
          ...
          <>
            {/* Score by difficulty */}
            <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:22,marginBottom:16}}>
            ...
            {/* Score trend */}
            <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:22}}>
...
function ManagerDashboard({go, userId=null}){
  ...
  return(
    <Shell page="team" go={go} userRole="manager" panel={false}>
      <div style={{animation:"fadeUp 0.35s ease both",maxWidth:900}}>
        ...
        {/* Tabs */}
        <div style={{display:"flex",gap:4,marginBottom:20,background:C.white,borderRadius:12,padding:4,border:`1px solid ${C.border}`,overflowX:"auto"}}>
          {tabs.map(t=>(
        ...
        {activeTab==="overview" && (
          <div style={{animation:"fadeUp 0.3s ease both"}}>
            {teamData.length===0 ? <EmptyTeam/> : (
              <>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:20}}>
                  {[
                    {label:"Team size",     val:teamData.length,                                          icon:"👥"},
                    {label:"Unread items",  val:inboxItems.filter(i=>!i.read).length,                    icon:"📬"},
                    {label:"Completed RPs", val:inboxItems.filter(i=>i.type==="roleplay_complete").length,icon:"🎭"},
                  ].map((s,i)=>(
                    <div key={i} style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:18,textAlign:"center"}}>
                      <div style={{fontSize:22,marginBottom:4}}>{s.icon}</div>
                      <div style={{fontSize:24,fontWeight:900,color:C.navy,lineHeight:1}}>{s.val}</div>
                      <div style={{fontSize:11,color:C.muted,marginTop:4}}>{s.label}</div>
                    </div>
                  ))}
                </div>
                <div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:18}}>
                  <div style={{fontSize:12,fontWeight:700,color:C.navy,marginBottom:12}}>Team roster</div>
                  {teamData.map((m,i)=>(
                    <div key={m.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<teamData.length-1?`1px solid ${C.bg}`:"none"}}>
                      <Av ini={(m.name||"?").slice(0,2).toUpperCase()} col={C.purple} sz={34}/>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,color:C.navy,fontSize:13}}>{m.name}</div>
                        <div style={{fontSize:11,color:C.muted}}>{m.role||"Recruiter"} · {m.focus||"General"}</div>
                      </div>
                      <button onClick={()=>removeMember(m.id)} style={{background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:12,padding:"4px 8px"}}>✕</button>
                    </div>
                  ))}
                </div>
              </>
                        ...
                        <div style={{padding:"0 18px 18px",borderTop:`1px solid ${C.bg}`}}>
                          {isRP?(
                            <div style={{marginTop:14}}>
                              {item.coachSummary&&<div style={{background:C.lavPale,borderRadius:10,padding:"10px 14px",border:`1px solid ${C.lavSoft}`,marginBottom:12}}><div style={{fontSize:10,fontWeight:700,color:C.purple,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Scott's summary</div><p style={{fontSize:12,color:C.navy,lineHeight:1.6,margin:0}}>{item.coachSummary}</p></div>}
                              {item.strongMoment&&<div style={{background:"#F0FDF4",borderRadius:10,padding:"10px 14px",border:"1px solid #BBF7D0",marginBottom:8}}><div style={{fontSize:10,fontWeight:700,color:C.green,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>✓ Best moment</div><p style={{fontSize:12,color:"#14532D",lineHeight:1.5,margin:0,fontStyle:"italic"}}>"{item.strongMoment.quote||item.strongMoment}"</p>{item.strongMoment.howToRepeat&&<p style={{fontSize:11,color:"#166534",margin:"4px 0 0"}}>{item.strongMoment.howToRepeat}</p>}</div>}
                              {item.missedMoment&&<div style={{background:"#FEF2F2",borderRadius:10,padding:"10px 14px",border:"1px solid #FECACA",marginBottom:8}}><div style={{fontSize:10,fontWeight:700,color:C.red,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>⚠ Missed</div>{item.missedMoment.candidateLine&&<p style={{fontSize:11,color:C.muted,fontStyle:"italic",margin:"0 0 3px"}}>Them: "{item.missedMoment.candidateLine}"</p>}<p style={{fontSize:12,color:"#7F1D1D",margin:0}}>{item.missedMoment.whatWasLost||item.missedMoment}</p>{item.missedMoment.betterResponse&&<p style={{fontSize:11,color:C.green,margin:"4px 0 0",fontWeight:600}}>→ "{item.missedMoment.betterResponse}"</p>}</div>}
                              {item.nextFocus&&<div style={{background:C.navy,borderRadius:10,padding:"10px 14px"}}><div style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>Work on next</div><p style={{fontSize:12,color:"rgba(255,255,255,0.85)",margin:0}}>{item.nextFocus}</p></div>}
                            </div>
                          ):(
                            <div style={{marginTop:12}}>
                              ...
                              <div style={{background:C.bg,borderRadius:8,padding:"10px 12px",fontSize:13,color:C.text,lineHeight:1.65}}>{item.text||<span style={{color:C.muted,fontStyle:"italic"}}>No response</span>}</div>
                            </div>
                          )}
                        </div>
                      )}
        ...
        {activeTab==="skills" && (
          <div style={{animation:"fadeUp 0.3s ease both"}}>
            {teamData.length===0 ? <EmptyTeam message="Add team members to track skill scores"/> : (
              <div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:22}}>
                <div style={{fontSize:13,fontWeight:700,color:C.navy,marginBottom:16}}>Skills overview — team data</div>
                <p style={{fontSize:13,color:C.muted,lineHeight:1.65}}>Skill scores populate automatically from completed roleplays. Encourage your team to complete all roleplay scenarios to build a complete picture.</p>
                <div style={{marginTop:16,display:"flex",flexDirection:"column",gap:10}}>
                  {teamData.map(m=>(
                    <div key={m.id} style={{display:"flex",alignItems:"center",gap:12,background:C.bg,borderRadius:10,padding:"10px 14px"}}>
                      <Av ini={(m.name||"?").slice(0,2).toUpperCase()} col={C.purple} sz={28}/>
                      <div style={{flex:1}}>
                        <div style={{fontSize:12,fontWeight:700,color:C.navy,marginBottom:3}}>{m.name}</div>
                        <div style={{fontSize:11,color:C.muted}}>{m.score||0} avg score · {m.roleplaysCount||0} roleplays</div>
                      </div>
                      <div style={{width:60,height:6,background:C.border,borderRadius:999,overflow:"hidden"}}><div style={{height:"100%",width:`${m.score||0}%`,background:m.score>=75?C.green:m.score>=60?C.amber:C.red,borderRadius:999}}/></div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {activeTab==="emotional" && (
          <div style={{animation:"fadeUp 0.3s ease both"}}>
            <div style={{background:C.lavPale,borderRadius:14,border:`1px solid ${C.lavSoft}`,padding:"14px 18px",marginBottom:16,display:"flex",gap:10,alignItems:"flex-start"}}>
              <span style={{fontSize:16}}>🧠</span>
              <p style={{fontSize:12,color:C.purple,lineHeight:1.65,margin:0}}>Synthesised from reflection responses and behavioural patterns. Use as coaching conversation starters — not clinical assessments.</p>
            </div>
            {teamData.length===0 ? <EmptyTeam message="Add team members to track emotional readiness"/> : (
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {teamData.map(m=>(
                  <div key={m.id} style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:18}}>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                      <Av ini={(m.name||"?").slice(0,2).toUpperCase()} col={C.purple} sz={32}/>
                      <div>
                        <div style={{fontWeight:700,color:C.navy,fontSize:13}}>{m.name}</div>
                        <div style={{fontSize:11,color:C.muted}}>{m.role||"Recruiter"}</div>
                      </div>
                    </div>
                    {m.emotionalReadiness?.summary ? (
                      <p style={{fontSize:13,color:C.text,lineHeight:1.65,margin:"0 0 10px"}}>{m.emotionalReadiness.summary}</p>
                    ):(
                      <p style={{fontSize:13,color:C.muted,fontStyle:"italic",margin:"0 0 10px"}}>No readiness data yet — complete reflections and roleplays to build this profile.</p>
                    )}
                    {m.emotionalReadiness?.conversationOpener&&(
                      <div style={{background:C.bg,borderRadius:8,padding:"8px 12px",border:`1px solid ${C.border}`}}>
                        <div style={{fontSize:10,fontWeight:700,color:C.purple,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>Conversation starter</div>
                        <p style={{fontSize:12,color:C.navy,lineHeight:1.5,margin:0,fontStyle:"italic"}}>"{m.emotionalReadiness.conversationOpener}"</p>
                      </div>
                    )}
                  </div>
                ))}
                  ...
                  <div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:18}}>
                    ...
                    {selLearner.smartGoals?.length ? (
                      selLearner.smartGoals.map((g,i)=>(
                        <div key={i} style={{background:C.bg,borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            ...
            <div style={{background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:20}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={()=>{saveManagerEmail(managerEmail);}}
                  style={{background:C.purple,color:"#fff",border:"none",borderRadius:8,padding:"9px 16px",fontWeight:700,fontSize:12,cursor:"pointer"}}>
                  Save
                </button>
              </div>
            </div>
        ...
        <Modal onClose={()=>setAddModal(false)}>
          ...
          {[{f:"name",p:"Full name *"},{f:"role",p:"Job title"},{f:"focus",p:"Recruitment focus"},{f:"billings",p:"Annual billings"}].map(({f,p})=>(
            <input key={f} value={newMember[f]||""} onChange={e=>setNewMember(prev=>({...prev,[f]:e.target.value}))}
              placeholder={p}
              style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",fontSize:13,color:C.text,outline:"none",marginBottom:8,boxSizing:"border-box"}}/>
          ))}
  ...
  return(
    <Shell page="team" go={go} userRole="manager" panel={false}>
      <div style={{maxWidth:520,margin:"0 auto"}}>
        ...
        <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:"24px 22px",marginBottom:16}}>
          ...
          {current.type==="multi" ? (
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {data.objections.map((obj,i)=>(
                <input key={i} value={obj}
                  ...
                  style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"11px 14px",fontSize:13,color:C.text,outline:"none",fontFamily:"'DM Sans',sans-serif"}}
                />
              ))}
            </div>
          ) : (
            <textarea value={data[current.field]||""} onChange={e=>update(current.field,e.target.value)}
              ...
              style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"11px 14px",fontSize:13,color:C.text,outline:"none",resize:"vertical",fontFamily:"'DM Sans',sans-serif",boxSizing:"border-box"}}
            />
          )}
        </div>
